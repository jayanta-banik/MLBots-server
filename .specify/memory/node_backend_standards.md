...
- Routes MUST be thin.
- Routes MUST handle request parsing and validation, auth or authorization
	wiring through middleware, and response shaping.
...
- Services MUST NOT return HTTP-specific concepts such as status codes.
- Services MUST return only the business payload.
