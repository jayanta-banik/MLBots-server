...
export const getObject = ({ bucketName, objectKey }) => {
  if (!bucketName || !objectKey) throw new Error('Bucket name and object key are required.');
...
export const uploadObject = async ({ bucketName, data, objectKey, contentType }) => {
  ...
  if (!bucketName || !data || !objectKey) throw new Error('Bucket name, data, and object key are required.');
